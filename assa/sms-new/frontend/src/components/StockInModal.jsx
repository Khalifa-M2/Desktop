...
import { createStockIn, updateStockIn, getAllItems } from "../api/apiClient";

const StockInModal = ({ record, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (record) {
      setFormData({
        ...
        StockInDate: record.StockInDate.split("T")[0],
      });
    }
  }, [record]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (record) {
        await updateStockIn(record._id, formData);
      } else {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 max-h-screen overflow-y-auto">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">
          {record ? "Edit Stock In" : "Add Stock In"}
        </h2>
