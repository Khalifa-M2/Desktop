...
import { getStockInRecords, deleteStockIn } from "../api/apiClient";
...
const StockInPage = () => {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const response = await getStockInRecords({ itemName: searchTerm });
      setRecords(response.data.stockInRecords);
    } catch (err) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Stock In Records</h1>
      ...
      {showModal && (
        <StockInModal record={selectedRecord} onClose={handleModalClose} />
      )}
            ...
            {records.length > 0 ? (
              records.map((record) => (
                <tr key={record._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">{record.QuantityIn}</td>
